...
class ModuleItem
{
 public:
    ...
    */
   explicit ModuleItem(const std::shared_ptr<newVcd::Module>& module, ModuleItem* parentItem = nullptr);
    ...
    * @return Константная ссылка на указатель std::shared_ptr<newVcdModule>.
    */
   const std::shared_ptr<newVcd::Module>&
   GetModule() const;

 private:
   std::shared_ptr<newVcd::Module> m_module; ///< Указатель на модуль.
...
class ModuleTreeModel : public QAbstractItemModel
{
 ...
 signals:
    ...
    */
   void ModuleClicked(std::shared_ptr<newVcd::Module> module);
 ...
 private:
    ...
    */
   void
   SetupModelData(const std::vector<std::shared_ptr<newVcd::Module>>& modules, ModuleItem* parent);

   ModuleItem* m_rootItem;                   ///< Корневой элемент дерева модулей.
   std::shared_ptr<newVcd::Handle> m_handle; ///< Текущий дескриптор VCD.
};

Q_DECLARE_METATYPE(std::shared_ptr<newVcd::Module>)
